#!/usr/bin/env python3
"""
This cron script goes through daily subscriptions and creates a CSV file with subscribed data and sends an email to the
subscriber.
"""
...
def main ():
    ...
    span_day = datetime.timedelta (days=1)
    last_day = (
        datetime.datetime (now.year, now.month, now.day, 0, 0, 0, 0) - span_day,
        datetime.datetime (now.year, now.month, now.day, 0, 0, 0, 0)
    )
    print ('from ' + last_day [0].isoformat () + ' to ' + last_day [1].isoformat())
    ...
    for subscription_row in subscription_db.cursor:
        if subscription_row [2] == 2:
            csv_filename, has_data = create_csv_file (sensor_data_db.cursor, subscription_row, last_day)
            if has_data:
                send_email_data (email_server, subscription_row [0], subscription_row [1], last_day, csv_filename)
            else:
